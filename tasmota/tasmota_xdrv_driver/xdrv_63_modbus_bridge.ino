...
#define D_CMND_MODBUS_LOW_POWER "LowPower"
...
const char kModbusBridgeCommands[] PROGMEM = "Modbus|" // Prefix
    D_CMND_MODBUS_SEND "|" D_CMND_MODBUS_SETBAUDRATE "|" D_CMND_MODBUS_SETSERIALCONFIG "|" D_CMND_MODBUS_LOW_POWER;

void (*const ModbusBridgeCommand[])(void) PROGMEM = {
  &CmndModbusBridgeSend, &CmndModbusBridgeSetBaudrate, &CmndModbusBridgeSetConfig, &CmndModbusBridgeLowPower
};
#endif
...
const char kModbusBridgeCommands[] PROGMEM = "Modbus|" // Prefix
    D_CMND_MODBUS_TCP_START "|" D_CMND_MODBUS_TCP_CONNECT "|" D_CMND_MODBUS_TCP_MQTT "|" D_CMND_MODBUS_SEND "|" D_CMND_MODBUS_SETBAUDRATE "|" 
    D_CMND_MODBUS_SETSERIALCONFIG "|" D_CMND_MODBUS_LOW_POWER;
...
void ModbusBridgeHandle(void)
{
  ...
  if (data_ready)
  {
    ...
    if (modbusBridge.deviceAddress != (uint8_t)modbusBridge.buffer[0])
...
void ModbusBridgeInit(void)
{
  ...
  {
    modbusBridgeModbus = new TasmotaModbus(Pin(GPIO_MBR_RX), Pin(GPIO_MBR_TX), Pin(GPIO_MBR_RE), Pin(GPIO_MBR_DE));
    ModbusBridgeBegin();
#ifdef USE_MODBUS_BRIDGE_TCP
...
void CmndModbusBridgeSend(void)
{
  ...
  if ((functionCode > (uint8_t)ModbusBridgeFunctionCode::mb_writeSingleRegister) &&
  ...
  else if (strcmp(stype, "float") == 0)
  {
    ...
    modbusBridge.dataCount = bitMode ? 2 * modbusBridge.count : 2 * modbusBridge.count;
  }
...
void CmndModbusBridgeLowPower(void) 
{
  modbusBridgeModbus->LowPower();
  ResponseCmndDone();
}
