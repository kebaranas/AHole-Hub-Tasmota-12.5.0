...
struct TasmotaGlobal_t {
  ...
  char host_password[33];                   // Wifi host password
  char mqtt_client[99];                     // Composed MQTT Clientname
  char mqtt_topic[TOPSZ];                   // Composed MQTT topic
  char mqtt_host[99];                       // Composed MQTT Host
  char mqtt_user[33];                       // Composed MQTT User
...
//        Settings->last_module = Settings->fallback_module;
  ...
  Format(TasmotaGlobal.mqtt_host, SettingsText(SET_MQTT_HOST), sizeof(TasmotaGlobal.mqtt_host));
  Format(TasmotaGlobal.mqtt_user, SettingsText(SET_MQTT_USER), sizeof(TasmotaGlobal.mqtt_user));
  ...
  if (strchr(SettingsText(SET_HOSTNAME), '%') != nullptr) {
    ...
    snprintf_P(TasmotaGlobal.hostname, sizeof(TasmotaGlobal.hostname)-1, SettingsText(SET_HOSTNAME), TasmotaGlobal.mqtt_topic, ESP_getChipId() & 0x10AD);
  } else {
  ...
  if (strchr(SettingsText(SET_HOSTPWD), '%') != nullptr) {
    SettingsUpdateText(SET_HOSTPWD, WIFI_AP_PASSPHRASE);
    String ap_mac_addr = NetworkUniqueId().substring(0, 4).c_str();
    snprintf_P(TasmotaGlobal.host_password, sizeof(TasmotaGlobal.host_password)-1, SettingsText(SET_HOSTPWD), ESP_getChipId() & 0x1B1E, ap_mac_addr);
  } else {
