...
const char kTasmotaCommands[] PROGMEM = "|"  // No prefix
  ...
  D_CMND_IPADDRESS "|" D_CMND_NTPSERVER "|" D_CMND_AP "|" D_CMND_SSID "|" D_CMND_PASSWORD "|" D_CMND_HOSTNAME "|" D_CMND_HOSTPASSWORD "|" D_CMND_WIFICONFIG "|" D_CMND_WIFI "|" D_CMND_DNSTIMEOUT "|"
...
void (* const TasmotaCommand[])(void) PROGMEM = {
  ...
  &CmndIpAddress, &CmndNtpServer, &CmndAp, &CmndSsid, &CmndPassword, &CmndHostname, &CmndHostPassword, &CmndWifiConfig, &CmndWifi, &CmndDnsTimeout,
...
void CmndHostPassword(void) {
  if (!XdrvMailbox.grpflg && (XdrvMailbox.data_len >= 0)) {
    SettingsUpdateText(SET_HOSTPWD, (SC_DEFAULT == Shortcut()) ? WIFI_AP_PASSPHRASE : SettingsText(SET_HOSTPWD));
    if (strlen(XdrvMailbox.data) >= 8) {
      SettingsUpdateText(SET_HOSTPWD, XdrvMailbox.data);
    }
  ...
  ResponseCmndChar(SettingsText(SET_HOSTPWD));
}
