...
#define TASMOTA_MODBUS_RE_DE_ENABLE        // Use local Tx enable on write buffer

TasmotaModbus::TasmotaModbus(int receive_pin, int transmit_pin, int re_enable_pin, int de_enable_pin) : TasmotaSerial(receive_pin, transmit_pin, 2)
{
#ifdef TASMOTA_MODBUS_RE_DE_ENABLE
  mb_re_enable_pin = re_enable_pin;     // Use local Tx enable on write buffer
  mb_de_enable_pin = de_enable_pin;
#else
  setReceiverEnablePin(re_enable_pin);  // Use TasmotaSerial Re enable on write byte
  setDriverEnablePin(de_enable_pin);    // Use TasmotaSerial De enable on write byte
...
int TasmotaModbus::Begin(long speed, uint32_t config)
{
  ...
  if (begin(speed, config)) {
    ...
    if (mb_re_enable_pin > -1) {
      pinMode(mb_re_enable_pin, OUTPUT);
      digitalWrite(mb_re_enable_pin, LOW);
    }
    if (mb_de_enable_pin > -1) {
      pinMode(mb_de_enable_pin, OUTPUT);
