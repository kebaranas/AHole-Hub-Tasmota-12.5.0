...
void TasmotaSerial::setReceiverEnablePin(int re_enable_pin) {
  if ((re_enable_pin > -1) && isValidGPIOpin(re_enable_pin)) {
    m_re_enable_pin = re_enable_pin;
    pinMode(m_re_enable_pin, OUTPUT);
    digitalWrite(m_re_enable_pin, LOW);
  }
}

void TasmotaSerial::setDriverEnablePin(int de_enable_pin) {
  if ((de_enable_pin > -1) && isValidGPIOpin(de_enable_pin)) {
    m_de_enable_pin = de_enable_pin;
    pinMode(m_de_enable_pin, OUTPUT);
    digitalWrite(m_de_enable_pin, LOW);
  }
}
...
size_t TasmotaSerial::write(uint8_t b) {
  ...
  if (m_re_enable_pin > -1) {
    digitalWrite(m_re_enable_pin, HIGH);
  }
  if (m_de_enable_pin > -1) {
    digitalWrite(m_de_enable_pin, HIGH);
  }
  size_t size = 0;
  if (m_hardserial) {
#ifdef ESP8266
